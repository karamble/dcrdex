...
// logSupportedNetworks fetches and logs the supported networks for USDT and USDC
// including both withdrawal addresses and deposit addresses for debugging.
func (m *mexc) logSupportedNetworks(ctx context.Context) {
	// Query coin info from MEXC to see withdrawal-enabled networks
	coinInfos, err := m.getCoinInfo(ctx)
	if err != nil {
		m.log.Warnf("Could not fetch coin info: %v", err)
	} else {
		m.log.Infof("MEXC: Fetched info for %d coins", len(coinInfos))
		// Log withdrawal-enabled networks for USDT and USDC
		for _, coinInfo := range coinInfos {
			if coinInfo.Coin == "USDT" || coinInfo.Coin == "USDC" {
				m.log.Infof("  %s withdrawal networks:", coinInfo.Coin)
				for _, net := range coinInfo.NetworkList {
					if net.WithdrawEnable {
						m.log.Infof("    - %s (min: %s, fee: %s)", net.Network, net.WithdrawMin, net.WithdrawFee)
					}
				}
			}
		}
	...
	// Only log for stablecoins we care about
	coinsToCheck := []string{"USDT", "USDC"}

	// Map coin symbols to their DEX asset IDs on different networks
	assetIDsToCheck := map[string][]uint32{
		"USDT": {60002, 966004}, // USDT on ETH, USDT on POLYGON
		"USDC": {60001, 966001}, // USDC on ETH, USDC on POLYGON
	}

	for _, coin := range coinsToCheck {
		m.log.Infof("MEXC: === Checking %s ===", coin)

		// 1. Check withdrawal addresses (user-configured destination addresses)
		// These are addresses the user has added to their MEXC account for withdrawals
		query := url.Values{}
		query.Add("coin", coin)
		...
		if err := m.getAPI(ctx, "/api/v3/capital/withdraw/address", query, true, &withdrawResp); err != nil {
			m.log.Debugf("  Could not fetch withdrawal addresses for %s: %v", coin, err)
		} else if len(withdrawResp.Data) == 0 {
			m.log.Infof("  Withdrawal addresses: None configured (user must add via MEXC UI)")
		} else {
			m.log.Infof("  Withdrawal addresses: %d configured", len(withdrawResp.Data))
			for _, item := range withdrawResp.Data {
				m.log.Infof("    - Network: %s, Address: %s", item.Network, item.Address)
			}
		}

		// 2. Try to fetch deposit addresses (addresses for receiving funds to MEXC)
		// Use GetDepositAddress for each asset ID to test the actual function the bot uses
		m.log.Infof("  Deposit addresses (for receiving to MEXC):")
		assetIDs := assetIDsToCheck[coin]
		for _, assetID := range assetIDs {
			symbol := dex.BipIDSymbol(assetID)
			network, err := m.getNetworkForAsset(assetID)
			if err != nil {
				m.log.Debugf("    Asset %s (ID %d): Could not determine network - %v", symbol, assetID, err)
				continue
			}

			address, err := m.GetDepositAddress(ctx, assetID)
			if err != nil {
				if strings.Contains(err.Error(), "no deposit address returned") {
					m.log.Infof("    Network %s: No deposit address available", network)
					m.debugAlternativeNetworks(ctx, coin, network)
				} else {
					m.log.Debugf("    Network %s: Error - %v", network, err)
				}
			} else {
				m.log.Infof("    Network %s: %s", network, address)
			}
		}
		m.log.Infof("")
	}
...
// debugAlternativeNetworks tries alternative network names and queries for debugging
func (m *mexc) debugAlternativeNetworks(ctx context.Context, coin, network string) {
	// Try without network to see if any addresses exist for this coin
	queryNoNet := url.Values{}
	queryNoNet.Add("coin", coin)
	var allAddrs []mxctypes.DepositAddressResponse
	if err := m.getAPI(ctx, "/api/v3/capital/deposit/address", queryNoNet, true, &allAddrs); err == nil && len(allAddrs) > 0 {
		// See if the desired network exists among returned entries
		found := false
		for _, addr := range allAddrs {
			if strings.EqualFold(addr.Network, network) {
				m.log.Debugf("      Found via no-network query: %s (tag: %s)", addr.Address, addr.Tag)
				found = true
				break
			}
		}
		if !found {
			m.log.Debugf("      %s not in no-network list. Found: %d entries", network, len(allAddrs))
			for i, addr := range allAddrs {
				if i < 3 { // Limit output
					m.log.Debugf("        - %s: %s", addr.Network, addr.Address)
				}
			}
		}
	} else {
		m.log.Debugf("      No deposit addresses even without network filter")
	}

	// Try common alias network names
	aliasMap := map[string][]string{
		"MATIC": {"POLYGON", "POLYGON_POS", "POL"},
		"ETH":   {"ERC20", "ETHEREUM"},
		"TRX":   {"TRC20", "TRON"},
		"BSC":   {"BEP20(BSC)", "BEP20", "BINANCE_SMART_CHAIN"},
	}
	aliases := aliasMap[network]
	for _, alt := range aliases {
		altQ := url.Values{}
		altQ.Add("coin", coin)
		altQ.Add("netWork", alt) // Note: MEXC uses "netWork" (capital W) as of 2024-06-09
		var altResp []mxctypes.DepositAddressResponse
		if err := m.getAPI(ctx, "/api/v3/capital/deposit/address", altQ, true, &altResp); err == nil && len(altResp) > 0 {
			for _, addr := range altResp {
				m.log.Debugf("      Found via alias %s: %s (tag: %s)", addr.Network, addr.Address, addr.Tag)
			}
			return
		}
	}
...
func (m *mexc) Connect(ctx context.Context) (*sync.WaitGroup, error) {
	...
	// Debug: Log supported networks for USDT/USDC
	m.logSupportedNetworks(ctx)
...
func (m *mexc) Markets(ctx context.Context) (map[string]*Market, error) {
	...
	for symbol, symInfo := range m.symbolMeta {
		// Skip non-trading symbols (Status: "1" = ENABLED, "0" = DISABLED)
		if symInfo.Status != "1" {
			skippedCount++
			continue
		}

		// Get all DEX asset IDs for base and quote assets
		...
		// Create ALL combinations of base and quote IDs, just like Coinbase does.
	...
	m.log.Infof("MEXC: Loaded %d DEX markets from %d MEXC symbols (skipped:%d disabled, %d no base support, %d no quote support)",
		len(markets), len(m.symbolMeta), skippedCount, noBaseCount, noQuoteCount)
	return markets, nil
}
...
func (m *mexc) ensureMarketConnection(ctx context.Context) error {
	...
	m.marketWS = marketWS
	marketWSWg, err := m.marketWS.Connect(m.ctx)
	if err != nil {
...
func (m *mexc) handleLimitDepthUpdate(wrapper *pb.PushDataV3ApiWrapper) {
	...
	// Send to book's update queue (non-blocking)
	select {
	case book.updateQueue <- update:
	default:
		m.log.Warnf("Update queue full for %s, dropping update", symbol)
	}
}
...
func (m *mexc) onMarketReconnect() {
	...
	if len(topics) == 0 {
		m.log.Debugf("Market WS reconnected, no topics to resubscribe")
		return
	}
