...
const (
	...
	bitgetOrderStatusPartialFill = "partial_fill"
	bitgetOrderStatusFullFill    = "full_fill"
	bitgetOrderStatusCancelled   = "cancelled"
...
func (bg *bitget) getOrderbookSnapshot(ctx context.Context, symbol string) (*bgtypes.OrderbookSnapshot, error) {
	...
	bg.log.Debugf("Got %s order book snapshot", symbol)
	return &snapshot, nil
}
...
func (bg *bitget) Markets(ctx context.Context) (map[string]*Market, error) {
	...
	tickerMap := make(map[string]*bgtypes.TickerData)
	for _, t := range tickers {
		tickerMap[t.Symbol] = t
	}

	m := make(map[string]*Market, len(matches))
	for _, mkt := range matches {
		ticker, found := tickerMap[mkt.Slug]
		if !found {
			continue
		}
...
func (bg *bitget) TradeStatus(ctx context.Context, tradeID string, baseID, quoteID uint32) (*Trade, error) {
	...
	var orderDetail bgtypes.OrderDetail
	...
	return &Trade{
		...
		Complete:    orderDetail.Status == "full_fill" || orderDetail.Status == "cancelled",
		Market:      market,
	}, nil
}
